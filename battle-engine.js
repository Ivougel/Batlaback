...
function createBattleSide(items, classId, prepMeta = {}) {
  const side = {
    ...
    guaranteedCritHitsRemaining: 0,
    mutualHpThresholdFired: new Set(),
    foeHpThresholdFired: new Set(),
    debuffThresholdFired: new Set(),
    hearts: 0,
...
function spendBattleStamina(state, side, team, amount, item, foe) {
  if (amount <= 0) return;
  side.stamina = Math.max(0, side.stamina - amount);
  side.staminaSpendFlash = 0.42;
  side.lastStaminaSpend = amount;
  if (item) applyInvulnOnStaminaSpend(state, item, side, team, amount, foe);
...
function tryActivateBattleRage(side, state, team, sourceLabel) {
  ...
  if (rageEffect.cleanseDebuffs && typeof cleanseSideDebuffs === "function") {
    cleanseSideDebuffs(side, Number(rageEffect.cleanseDebuffs));
  }
  if (rageEffect.gainStack) {
    applyGainStackEffect(state, rageEffect.gainStack, null, side, team);
  }
  finalizeSideCombatStats(side);
  pushBattleLog(state, {
    actor: team,
...
function checkFoeHpThresholdEffects(state) {
  if (!state?.player || !state?.enemy) return;
  [
    { side: state.player, foe: state.enemy, team: "player" },
    { side: state.enemy, foe: state.player, team: "enemy" },
  ].forEach(({ side, foe, team }) => {
    if (!foe?.maxHp) return;
    const foeRatio = foe.hp / foe.maxHp;
    const selfRatio = side.maxHp > 0 ? side.hp / side.maxHp : 1;
    side.items.forEach((item) => {
      collectItemBattleEffects(item).forEach((effect) => {
        if (effect.type !== "foeHpThreshold") return;
        const threshold = Number(effect.threshold) || 0.5;
        const selfThreshold = Number(effect.selfThreshold);
        const foeOk = foeRatio <= threshold;
        const selfOk = Number.isFinite(selfThreshold) ? selfRatio <= selfThreshold : false;
        if (!foeOk && !selfOk) return;
        const key = getStackThresholdKey(item, effect);
        if (!side.foeHpThresholdFired) side.foeHpThresholdFired = new Set();
        if (effect.once !== false && side.foeHpThresholdFired.has(key)) return;
        if (effect.once !== false) side.foeHpThresholdFired.add(key);
        const rt = item.runtime || createRuntimeState(item);
        const def = ITEM_CATALOG[item.itemId];
        let dmg = Number(effect.damage) || 0;
        if (effect.damagePerFoeDebuffMult && typeof countSideDebuffs === "function") {
          dmg += countSideDebuffs(foe) * Number(effect.damagePerFoeDebuffMult);
        }
        if (dmg > 0 && foe) {
          const savedLs = side.lifesteal;
          if (effect.lifesteal) side.lifesteal = Number(effect.lifesteal);
        ...
        if (effect.gainStack) applyGainStackEffect(state, effect.gainStack, item, side, team);
        if (effect.heal) executeEffect(state, { type: "heal", value: effect.heal }, item, side, foe, rt, team);
        if (effect.gainDominantStack) {
          const stacks = ["spikes", "block", "empower", "regen", "luck", "heat", "mana", "cold"];
          let maxType = stacks[0];
          let maxVal = getSideStack(side, maxType);
          stacks.forEach((t) => {
          ...
          applyGainStackEffect(state, {
            stack: maxType,
            value: Number(effect.gainDominantStack) || 5,
          }, item, side, team);
        }
        if (state && def) {
          pushBattleLog(state, {
            ...
            type: "buff",
            source: def.name,
            message: `${battleTeamLabel(team)} · ${def.name}: порог HP противника`,
          });
        }
      });
    });
...
function checkDebuffThresholdEffects(state, side, foe, team) {
  if (!side?.items || typeof countSideDebuffs !== "function") return;
  const debuffs = countSideDebuffs(side);
  side.items.forEach((item) => {
    collectItemBattleEffects(item).forEach((effect) => {
      if (effect.type !== "debuffThreshold") return;
      const need = Number(effect.threshold) || 10;
      if (debuffs < need) return;
      const key = getStackThresholdKey(item, effect);
      if (!side.debuffThresholdFired) side.debuffThresholdFired = new Set();
      if (effect.once !== false && side.debuffThresholdFired.has(key)) return;
      if (effect.once !== false) side.debuffThresholdFired.add(key);
      const rt = item.runtime || createRuntimeState(item);
      const def = ITEM_CATALOG[item.itemId];
      if (effect.cleanseDebuffs && typeof cleanseSideDebuffs === "function") {
        cleanseSideDebuffs(side, effect.cleanseDebuffs);
      }
      if (effect.gainStack) applyGainStackEffect(state, effect.gainStack, item, side, team);
      if (effect.gainDominantStack) {
        ...
        applyGainStackEffect(state, {
          stack: maxType,
          value: Number(effect.gainDominantStack) || 8,
        }, item, side, team);
      }
      if (effect.convertHp) {
        const hpCost = Math.min(Number(effect.convertHp.hpCost) || 15, Math.max(0, side.hp - 1));
        if (hpCost > 0) {
          side.hp -= hpCost;
          applyGainStackEffect(state, {
            stack: effect.convertHp.stack || "regen",
            value: effect.convertHp.stackGain || 35,
          }, item, side, team);
        }
      }
      if (state && def) {
        pushBattleLog(state, {
          ...
          type: "buff",
          source: def.name,
          message: `${battleTeamLabel(team)} · ${def.name}: ${need}+ дебаффов`,
        });
      }
    });
  });
...
function applyGainStackEffect(state, effect, item, self, team, foe = null) {
  const stack = effect.stack || "spikes";
  if (effect.chance != null && Math.random() > Number(effect.chance)) return 0;
  let target = self;
  let targetTeam = team;
  if (effect.targetSide === "foe" && foe) {
    target = foe;
    targetTeam = team === "player" ? "enemy" : "player";
  }
  let amount = Math.max(1, Math.floor(Number(effect.value ?? 1)));
  if (self?.stackGainMult) amount = Math.ceil(amount * (1 + self.stackGainMult));
  (self?.items || []).forEach((it) => {
    collectItemBattleEffects(it).forEach((fx) => {
      if (fx.type === "stackGainMult") {
        amount = Math.ceil(amount * (1 + (Number(fx.value) || 0)));
      }
    });
  });
  const added = typeof addSideStack === "function"
    ? addSideStack(target, stack, amount, effect.cap)
    : 0;
  if (added <= 0) return 0;
  const def = ITEM_CATALOG[item.itemId];
  if (typeof syncStackResourceGain === "function") syncStackResourceGain(target, stack, added);
  else {
    if (stack === "block") target.block += added;
    if (stack === "luck") target.luck += added;
  }
  if (state && def && targetTeam) {
    ...
    pushBattleLog(state, {
      ...
      type: "buff",
      source: def.name,
      message: `${battleTeamLabel(targetTeam)} · ${def.name}: ${label} (всего: ${typeof getSideStack === "function" ? getSideStack(target, stack) : added})`,
    });
  ...
  if (state && team) {
    ...
    checkStackThresholds(state, watchSelf, watchFoe, team, item);
  }
  return added;
}

function stealRandomStackFromFoe(state, item, self, foe, team, amount = 1) {
  if (!foe || typeof pickRandomPositiveStack !== "function") return false;
  const stackType = pickRandomPositiveStack(foe);
  if (!stackType || !spendSideStack(foe, stackType, amount)) return false;
  if (stackType === "block" && typeof syncStackResourceSpend === "function") {
    syncStackResourceSpend(foe, stackType, amount);
  }
  applyGainStackEffect(state, { stack: stackType, value: amount }, item, self, team);
  return true;
}

function spendRandomSelfStack(state, item, self, team, amount = 1) {
  if (typeof pickRandomPositiveStack !== "function") return false;
  const stackType = pickRandomPositiveStack(self);
  if (!stackType || !spendSideStack(self, stackType, amount)) return false;
  if (typeof syncStackResourceSpend === "function") syncStackResourceSpend(self, stackType, amount);
  const def = ITEM_CATALOG[item?.itemId];
  if (state && def) {
    pushBattleLog(state, {
      actor: team,
      type: "debuff",
      source: def.name,
      message: `${battleTeamLabel(team)} · ${def.name}: −${amount} ${typeof getStackLabel === "function" ? getStackLabel(stackType, amount) : stackType}`,
    });
  }
  return true;
}
...
function checkStackThresholds(state, self, foe, team, sourceItem = null) {
  ...
  self.items.forEach((item) => {
    ...
    collectItemBattleEffects(item).forEach((effect) => {
      ...
      if (effect.guaranteedCritHits) {
        self.guaranteedCritHitsRemaining = Math.max(
          self.guaranteedCritHitsRemaining || 0,
          Number(effect.guaranteedCritHits) || 3,
        );
      }
      if (effect.convertHp) {
      ...
      if (effect.gainBlock) {
        applyGainStackEffect(state, { stack: "block", value: effect.gainBlock }, item, self, team);
      }
      if (effect.gainStack) applyGainStackEffect(state, effect.gainStack, item, self, team, foe);
  ...
  sides.forEach(({ self, foe, team }) => {
    self.items.forEach((item) => {
      ...
      effects.forEach((effect) => {
        ...
        if (effect.stripFoeStacksOnceEach && foe && typeof stripOneStackPerType === "function") {
          const stripped = stripOneStackPerType(foe);
          const def = ITEM_CATALOG[item.itemId];
          if (state && def && stripped > 0) {
            pushBattleLog(state, {
              actor: team,
              type: "debuff",
              source: def.name,
              message: `${battleTeamLabel(team)} · ${def.name}: снято ${stripped} стак(ов) у противника`,
            });
          }
        }
        if (effect.drainFoeStamina && foe) {
          const drain = Number(effect.drainFoeStamina) || 0;
          if (drain > 0) {
            foe.stamina = Math.max(0, (foe.stamina || 0) - drain);
            const def = ITEM_CATALOG[item.itemId];
            if (state && def) {
              pushBattleLog(state, {
                actor: team,
                type: "debuff",
                source: def.name,
                message: `${battleTeamLabel(team)} · ${def.name}: −${drain} выносливости противнику`,
              });
            }
          }
        }
        rt.periodicTriggerCount = (rt.periodicTriggerCount || 0) + 1;
        if (effect.stunEvery && foe && rt.periodicTriggerCount % Number(effect.stunEvery) === 0) {
          foe.stunTimer = Math.max(foe.stunTimer || 0, Number(effect.stunDuration) || 1);
          const def = ITEM_CATALOG[item.itemId];
          if (state && def) {
            pushBattleLog(state, {
              actor: team,
              type: "debuff",
              source: def.name,
              message: `${battleTeamLabel(team)} · ${def.name}: оглушение ${Number(effect.stunDuration) || 1}с`,
            });
          }
        }
        if (effect.cooldownPenalty) {
          item.currentCooldown = (item.currentCooldown || 0) + Number(effect.cooldownPenalty);
        }
        if (effect.stealFoeStack && foe && typeof spendSideStack === "function" && typeof addSideStack === "function") {
          const sf = effect.stealFoeStack;
          const stack = sf.stack || "heat";
          const amt = Number(sf.value) || 1;
          if (spendSideStack(foe, stack, amt)) {
            applyGainStackEffect(state, { stack, value: amt }, item, self, team);
          }
        }
        const hpRatio = self.maxHp > 0 ? self.hp / self.maxHp : 1;
        const hasHpBranch = effect.hpThreshold != null;
        if (hasHpBranch) {
          const threshold = Number(effect.hpThreshold) || 0.7;
          if (hpRatio >= threshold && effect.gainStackIfAbove) {
            applyGainStackEffect(state, effect.gainStackIfAbove, item, self, team, foe);
          } else if (hpRatio < threshold && effect.healIfBelow) {
            executeEffect(state, { type: "heal", value: effect.healIfBelow }, item, self, foe, rt, team);
          }
        } else if (effect.gainStack) {
          if (effect.chance == null || rollEffectChance(self, effect.chance)) {
            const gs = typeof effect.gainStack === "object" ? effect.gainStack : { stack: effect.gainStack, value: 1 };
            applyGainStackEffect(state, gs, item, self, team, foe);
          }
        }
        ...
        if (effect.applyColdOrSelf && foe) {
          const threshold = Number(effect.coldThreshold) || 10;
          if (getSideStack(foe, "cold") < threshold) {
            applyGainStackEffect(state, { stack: "cold", value: 1 }, item, self, team, foe);
          } else {
            applyGainStackEffect(state, { stack: "cold", value: 1 }, item, self, team);
          }
        }
        if (effect.foeColdBonus && foe && getSideStack(foe, "cold") >= (Number(effect.foeColdBonus.threshold) || 10)) {
          const bonus = effect.foeColdBonus;
          if (bonus.magicDamageMult) self.magicDamageMult *= 1 + Number(bonus.magicDamageMult);
          if (bonus.damage) {
            applyDamage(
              foe,
              scalePacedValue(Number(bonus.damage) || 10, DAMAGE_PACING_SCALE),
              state,
              ITEM_CATALOG[item.itemId]?.name || "Холод",
              team,
              self,
              { damageType: bonus.damageType || "magic" },
              item,
              { noRetaliation: true },
            );
          }
        ...
        if (effect.gainWeakestStack && typeof applyGainWeakestStack === "function") {
          const gw = effect.gainWeakestStack === true ? {} : effect.gainWeakestStack;
          const times = Number(gw.count) || 1;
          for (let i = 0; i < times; i += 1) {
            applyGainWeakestStack(state, self, item, team, gw);
          }
        }
        ...
        if (effect.gainAllStacks) {
          const amt = Number(effect.gainAllStacks) || 1;
          ["spikes", "block", "empower", "regen", "luck", "heat", "mana"].forEach((stack) => {
            applyGainStackEffect(state, { stack, value: amt }, item, self, team);
          });
        }
        if (effect.cooldownBoostItem) {
          let targetItem = null;
          let maxCd = -1;
          self.items.forEach((it) => {
            if ((it.currentCooldown || 0) > maxCd) {
              maxCd = it.currentCooldown || 0;
              targetItem = it;
            }
          });
          if (targetItem && maxCd > 0) {
            targetItem.currentCooldown = Math.max(0, maxCd * (1 - (Number(effect.cooldownBoostItem) || 0.06)));
          }
        }
        if (effect.maxHp) {
          self.maxHp += Number(effect.maxHp);
...
function checkHpThresholdEffects(state, side, foe, team) {
  ...
  side.items.forEach((item) => {
    collectItemBattleEffects(item).forEach((effect) => {
      ...
      if (effect.cooldownBoost) {
        side.cooldownMult *= Math.max(0.35, 1 - Number(effect.cooldownBoost));
        finalizeSideCombatStats(side);
      }
      if (effect.timedDamageReduction) {
        const tdr = typeof effect.timedDamageReduction === "object"
          ? effect.timedDamageReduction
          : { value: effect.timedDamageReduction, duration: 5 };
        side.timedBuffs.push({
          stat: "damageTaken",
          value: -(Number(tdr.value) || 0.20),
          remaining: Number(tdr.duration) || 5,
        });
      }
      if (state && def) {
        pushBattleLog(state, {
...
function applyInvulnOnStaminaSpend(state, item, side, team, staminaCost, foe) {
  collectItemBattleEffects(item).forEach((effect) => {
    ...
    const rt = item.runtime || createRuntimeState(item);
    const def = ITEM_CATALOG[item.itemId];
    if (effect.foeSlow && foe) {
      executeEffect(state, {
        type: "slow",
        value: Number(effect.foeSlow),
        duration: Number(effect.slowDuration) || 6,
      }, item, side, foe, rt, team);
    } else if ((effect.duration || 0) > 0) {
      grantSideInvulnerability(side, effect.duration || 2);
    }
    if (effect.gainStack) applyGainStackEffect(state, effect.gainStack, item, side, team);
    if (state && def) {
      const msg = effect.foeSlow
        ? `${battleTeamLabel(team)} · ${def.name}: замедление ${effect.foeSlow}`
        : `${battleTeamLabel(team)} · ${def.name}: неуязвимость ${effect.duration || 2}с`;
      pushBattleLog(state, { actor: team, type: "buff", source: def.name, message: msg });
    }
  });
}

function applyMaxHpPercentStart(side, ratio) {
  const bonus = Math.floor(side.maxHp * Math.max(0, Number(ratio) || 0));
  if (bonus <= 0) return;
  side.maxHp += bonus;
}

function countCardItemsOnSide(side) {
  return typeof countTaggedItemsOnSide === "function"
    ? countTaggedItemsOnSide(side, "card")
    : 0;
}

function countNeutralItemsOnSide(side) {
  return (side?.items || []).filter((item) => {
    const def = ITEM_CATALOG[item.itemId];
    return def && !def.isContainer && !def.classRestriction;
  }).length;
}

function isStoneItemId(itemId) {
  return itemId === "stone"
    || (typeof itemId === "string" && itemId.startsWith("artifact_stone_"));
}

function triggerOnFatigueStartEffects(state) {
  [
    ...
    side.items.forEach((item) => {
      collectItemBattleEffects(item).forEach((effect) => {
        if (effect.type !== "onFatigueStart") return;
        if (effect.gainStack) applyGainStackEffect(state, effect.gainStack, item, side, team);
      });
    });
  });
...
function applyCrossSideBattleStartEffects(state) {
  [
    { self: state.player, foe: state.enemy, team: "player" },
    { self: state.enemy, foe: state.player, team: "enemy" },
  ].forEach(({ self, foe, team }) => {
    self.items.forEach((item) => {
      const effects = collectItemBattleEffects(item);
      effects.forEach((effect) => {
        ...
        if (effect.bothSides && effect.type === "timedDamageReduction") {
          applyTimedDamageReductionStart(self, effect);
          applyTimedDamageReductionStart(foe, effect);
        }
        if (effect.bothSides && effect.type === "poison") {
          executeEffect(state, { type: "poison", value: effect.value || 1 }, item, self, self, rt, team);
          executeEffect(state, { type: "poison", value: effect.value || 1 }, item, foe, foe, rt, foeTeam);
        }
        if (effect.targetSide === "foe" && effect.type === "gainStack") {
          applyGainStackEffect(state, effect, item, self, team, foe);
        }
        if (effect.type === "stealRandomStack" && effect.trigger === "battle_start" && foe) {
          const times = Number(effect.value) || 1;
          for (let i = 0; i < times; i += 1) {
            stealRandomStackFromFoe(state, item, self, foe, team, 1);
          }
        }
        if (effect.type === "cardScaledDamage" && foe) {
          const cards = countCardItemsOnSide(self);
          const dmg = (Number(effect.base) || 0) + Math.max(0, cards - 1) * (Number(effect.perCard) || 0);
          if (dmg > 0) {
            executeEffect(state, {
              type: "damage",
              value: dmg,
              damageType: effect.damageType || "magic",
            }, item, self, foe, rt, team);
          }
        }
      });
    });
...
function applyBattleStartItemEffects(side) {
  ...
  side.items.forEach((item) => {
    ...
    effects.forEach((effect) => {
      ...
      if (effect.type === "maxHpPercentStart") applyMaxHpPercentStart(side, effect.value);
      if (effect.type === "passiveMaxHp") side.maxHp += Number(effect.value) || 0;
      if (effect.type === "gainWeakestStack" && typeof applyGainWeakestStack === "function") {
        const times = Number(effect.count) || 1;
        for (let i = 0; i < times; i += 1) {
          applyGainWeakestStack(null, side, item, null, { value: effect.value || 1 });
        }
      }
      if (effect.type === "cardScaledBonus") {
        const cards = countCardItemsOnSide(side);
        const bonus = Math.max(0, cards - 1) * (Number(effect.perCard) || 5);
        if (bonus > 0 && effect.stack) {
          applyGainStackEffect(null, { stack: effect.stack, value: bonus }, item, side, null);
        }
      }
      if (effect.type === "neutralScaledStack") {
        const count = countNeutralItemsOnSide(side);
        const per = Number(effect.perItem ?? effect.value) || 8;
        if (count > 0 && effect.stack) {
          applyGainStackEffect(null, { stack: effect.stack, value: count * per }, item, side, null);
        }
      }
      if (effect.type === "selfPoisonStart") {
        side.poisonStacks = (side.poisonStacks || 0) + (Number(effect.value) || 1);
      }
      if (effect.type === "heal" && Number(effect.value) > 0) {
        const rt = item.runtime || createRuntimeState(item);
        executeEffect(null, { type: "heal", value: effect.value }, item, side, null, rt, null);
      }
    });
  });
...
function processOnHitItemEffects(state, item, self, foe, team) {
  ...
  effects.forEach((effect) => {
    ...
    if (effect.type === "gainStack") applyGainStackEffect(state, effect, item, self, team, foe);
    if (effect.type === "spendStack") applySpendStackEffect(state, effect, item, self, foe, rt, team);
    if (effect.type === "applyStun") {
      if (effect.selfStaminaBelow != null && self.stamina >= Number(effect.selfStaminaBelow)) return;
    ...
    if (effect.type === "staminaSpendOnHit" && self.stamina >= (Number(effect.staminaCost) || 1)) {
      const cost = Number(effect.staminaCost) || 1;
      self.stamina -= cost;
      const bonus = Number(effect.itemDamage) || 1;
      rt.damageBonus = (rt.damageBonus || 0) + bonus;
      if (effect.weaponDamage) applyWeaponStartDamageBonus(self, effect.weaponDamage);
    }
    if (effect.type === "stealRandomStack" && foe && rollEffectChance(self, effect.chance ?? 1)) {
      stealRandomStackFromFoe(state, item, self, foe, team, Number(effect.value) || 1);
    }
    if (effect.type === "destroyFoeStacks" && foe) {
      if (typeof destroyRandomSideStacks === "function") {
        destroyRandomSideStacks(foe, Number(effect.value) || 4);
      }
    }
    if (effect.type === "bonusDamageOnHit" && rollEffectChance(self, effect.chance ?? 1)) {
      executeEffect(state, { type: "damage", value: Number(effect.value) || 1 }, item, self, foe, rt, team, { skipOnHit: true });
      if (effect.gainStack) applyGainStackEffect(state, effect.gainStack, item, self, team, foe);
    }
    if (effect.type === "fatigueDamageOnHit" && foe && typeof isFatigueActive === "function" && isFatigueActive(state)) {
      foe.fatigueLevel = (foe.fatigueLevel || 0) + (Number(effect.value) || 1);
      executeEffect(state, { type: "poison", value: Number(effect.poison) || 2 }, item, self, foe, rt, team, { skipOnHit: true });
    }
    if (effect.type === "gainWeakestStack" && typeof applyGainWeakestStack === "function") {
      applyGainWeakestStack(state, self, item, team, effect.gainWeakestStack === true ? {} : effect);
    }
  });
  checkStackThresholds(state, self, foe, team, item);
}
...
function applyPassiveEffect(side, item, effect) {
  switch (effect.type) {
    ...
    case "cooldownMultPerAdjacent": {
      ...
      const maxBonus = Number(effect.maxBonus ?? 0.60);
      const bonus = Math.min(maxBonus, per * neighbors.size);
      if (bonus > 0) {
        item.runtime = item.runtime || createRuntimeState(item);
        item.runtime.cooldownMult = clampCooldownMult((item.runtime.cooldownMult || 1) * (1 - bonus));
      }
      break;
    }
    case "cooldownMultPerItemCost": {
      ...
      side.items.forEach((it) => {
        const d = ITEM_CATALOG[it.itemId];
        if (d && !d.isContainer) totalCost += Number(d.cost) || 0;
      });
      const per = Number(effect.perCost ?? 0.01);
      const maxBonus = Number(effect.maxBonus ?? 0.99);
      const bonus = Math.min(maxBonus, per * totalCost);
      if (bonus > 0) {
        item.runtime = item.runtime || createRuntimeState(item);
        item.runtime.cooldownMult = clampCooldownMult((item.runtime.cooldownMult || 1) * (1 - bonus));
      }
    ...
    case "cooldownMultPerSocket": {
      let filled = 0;
      side.items.forEach((it) => {
        filled += (it.socketedGems?.filter(Boolean).length || 0);
      });
      const per = Number(effect.perSocket ?? 0.03);
      const maxBonus = Number(effect.maxBonus ?? 0.60);
      const bonus = Math.min(maxBonus, per * filled);
      if (bonus > 0) {
        side.cooldownMult = clampCooldownMult(side.cooldownMult * (1 - bonus));
      }
      break;
    }
    ...
    case "healAsDamageMult":
      side.healAsDamageMult = (side.healAsDamageMult || 0) + (Number(effect.value) || 0);
      break;
    case "stackGainMult":
      side.stackGainMult = (side.stackGainMult || 0) + (Number(effect.value) || 0);
      break;
    case "stonesMultiThrow":
      side.stonesMultiThrow = true;
      break;
    case "staminaRegenPerStack":
      if (typeof getTotalSideStacks === "function") {
        side.staminaRegen = (side.staminaRegen || STAMINA_REGEN_PER_SEC)
          + getTotalSideStacks(side) * (Number(effect.value) || 0.007);
      }
      break;
...
function createBattleState(playerItems, enemyItems, playerClassId = null, enemyClassId = null, battleRound = 1, prepMeta = {}) {
  ...
  if (typeof applyCrossSideBattleStartEffects === "function") applyCrossSideBattleStartEffects(state);
  state.replayFrames = [];
...
function getSideCooldownMult(side) {
  ...
  (side.items || []).forEach((item) => {
    collectItemBattleEffects(item).forEach((effect) => {
      if (effect.type === "cooldownMultPerTotalStacks" && typeof getTotalSideStacks === "function") {
        const perStack = Number(effect.perStack ?? effect.value) || 0.05;
        const stackCount = Math.min(
          Number(effect.maxStacks) || 999,
          getTotalSideStacks(side),
        );
        mult *= Math.max(0.35, 1 - perStack * stackCount);
      }
    });
  });
  return mult;
...
function getEffectiveCritChance(side, item, foe) {
  ...
  if ((side.guaranteedCritHitsRemaining || 0) > 0) return 1;
  ...
  (side.items || []).forEach((it) => {
    collectItemBattleEffects(it).forEach((effect) => {
      ...
      if (effect.type === "critPerFoeFatigue" && foe) {
        crit += (foe.fatigueLevel || 0) * (Number(effect.value) || 0.07);
      }
    });
  });
...
function battleTick(state, dt) {
  ...
  sides.forEach(({ self, foe, team }) => {
    ...
    ready.forEach((item) => {
      ...
      if (staminaCost > 0) {
        spendBattleStamina(state, self, team, staminaCost, item, foe);
      }
...
function tickStatusEffects(state, dt) {
  ...
  checkFoeHpThresholdEffects(state);
  [state.player, state.enemy].forEach((side, idx) => {
    const team = idx === 0 ? "player" : "enemy";
    const foe = idx === 0 ? state.enemy : state.player;
    checkDebuffThresholdEffects(state, side, foe, team);
...
function tickFatigue(state, dt) {
  ...
  if (!state.fatigueAnnounced) {
    ...
    if (typeof triggerOnFatigueStartEffects === "function") triggerOnFatigueStartEffects(state);
  }
...
function triggerOnActivateListeners(state, sourceItem, self, foe, team) {
  ...
  self.items.forEach((listenerItem) => {
    ...
    effects.forEach((effect) => {
      ...
      if (effect.foePoison && foe && rollEffectChance(self, effect.chance ?? 1)) {
        executeEffect(state, { type: "poison", value: effect.foePoison }, listenerItem, self, foe, rt, team);
      }
    });
  });
}
...
function activateItem(state, item, self, foe, team) {
  ...
  activeEffects.forEach((effect) => {
    if (effect.type === "groundFire" || effect.type === "dodgePeriodic" || effect.type === "repeatCast"
      ...
      || effect.type === "cooldownMultPerAdjacent" || effect.type === "cooldownMultPerItemCost"
      || effect.type === "cooldownMultPerSocket" || effect.type === "cooldownMultPerTotalStacks"
      ...
      || effect.type === "onActivate" || effect.type === "foeHpThreshold" || effect.type === "debuffThreshold"
      || effect.type === "procChanceBonus" || effect.type === "damagePerTotalStacks") return;
    executeEffect(state, effect, item, self, foe, rt, team);
  });
  ...
  if (rt.activationsLeft != null) {
    const skipStoneLimit = self.stonesMultiThrow && isStoneItemId(def?.id);
    if (!skipStoneLimit) {
      rt.activationsLeft = Math.max(0, rt.activationsLeft - 1);
    }
  }
  ...
  switch (effect.type) {
    case "damage": {
      ...
      if (foe?.maxHp > 0) {
        ...
        (self.items || []).forEach((it) => {
          collectItemBattleEffects(it).forEach((fx) => {
            if (fx.type === "foeHpThreshold" && foeRatio <= (Number(fx.threshold) || 0.3) && fx.damageMult) {
              base *= 1 + Number(fx.damageMult);
            }
          });
        });
      }
      ...
      if (critChance > 0 && Math.random() < critChance) {
        ...
        if ((self.guaranteedCritHitsRemaining || 0) > 0) {
          self.guaranteedCritHitsRemaining -= 1;
        }
        pushBattleLog(state, {
          actor: team,
          type: "crit",
    ...
    case "heal": {
      ...
      if (typeof getTagDamageBonus === "function") {
        ...
        if (healPerTagFx) {
          amount += getTagDamageBonus(
            self,
            healPerTagFx.tag,
            healPerTagFx.value,
            healPerTagFx.adjacent ? item : null,
          );
        }
      }
      ...
      if (healed > 0 && foe && (self.healAsDamageMult || 0) > 0) {
        const magicDmg = Math.floor(healed * self.healAsDamageMult);
        if (magicDmg > 0) {
          applyDamage(
            foe,
            scalePacedValue(magicDmg, DAMAGE_PACING_SCALE),
            state,
            def.name,
            team,
            self,
