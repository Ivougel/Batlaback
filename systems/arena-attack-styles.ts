// @ts-nocheck — каталог STYLES: сотни фазовых колбэков; публичный API типизирован через ArenaAttackStylesApi.
/**
 ...
 */
import type { ArenaAttackStylesApi } from "../types/game";
...
const ArenaAttackStyles = (() => {
  ...
  function quadBezier(p0: Point2D, p1: Point2D, p2: Point2D, t: number): Point2D {
    const u = 1 - t;
    return {
  ...
  function itemParams(itemId: string): ArenaItemParams {
  ...
  function arcCtrl(from: Point2D, to: Point2D, vmin: number, liftRatio = 0.14, sideBias = 0): Point2D {
    const midX = (from.x + to.x) * 0.5 + sideBias * vmin * 0.04;
    const midY = Math.min(from.y, to.y) - vmin * liftRatio;
    return { x: midX, y: midY };
  }

  function linearPhase(from: Point2D, to: Point2D, t: number, ease: EaseFn = easeOutCubic): ArenaAttackPose {
  ...
  function bezierPhase(from: Point2D, to: Point2D, ctrl: Point2D, t: number, ease: EaseFn = easeOutCubic): ArenaAttackPose {
    const u = ease(Math.min(1, Math.max(0, t)));
  ...
  function emojiAvatarArcCtrl(from: Point2D, to: Point2D, vmin: number): Point2D {
  ...
  function emojiAvatarStrikeArc(atk: ArenaAttackCtx, t: number, vmin: number, params: ArenaItemParams): ArenaAttackPose {
  ...
  function emojiAvatarRecoverArc(atk: ArenaAttackCtx, t: number, vmin: number, params: ArenaItemParams): ArenaAttackPose {
  ...
  const PROJECTILE_GLYPH: Record<string, string> = {
    arrow: "➶",
    javelin: "🗡️",
    fireball: "🔥",
    arcane: "✨",
  ...
  function isProjectileStyle(styleId: string): boolean {
    return PROJECTILE_STYLES.has(styleId);
  }

  function getProjectileGlyph(styleId: string, itemId: string): string {
    if (styleId === "fireball") return "🔥";
    if (styleId === "arcane") return "✨";
    return PROJECTILE_GLYPH[styleId] || "•";
  }

  function weaponHomePose(
    atk: ArenaAttackCtx,
    body: ArenaEquipBody,
    style: ArenaAttackStyleDef,
    vmin: number,
    params: ArenaItemParams,
  ): ArenaAttackPose {
    const homeX = atk.useEmojiAvatarArc
      ? (atk.homeVpX ?? body.homeX)
      : (atk.useViewport ? (atk.homeVpX ?? body.homeX) : body.homeX);
    const homeY = atk.useEmojiAvatarArc
      ? (atk.homeVpY ?? body.homeY)
      : (atk.useViewport ? (atk.homeVpY ?? body.homeY) : body.homeY);
    const holdAtk: ArenaAttackCtx = { ...atk, fromX: homeX, fromY: homeY };
  ...
  } as unknown as Record<string, ArenaAttackStyleDef>;
  ...
  function hasTag(def: CatalogDef | null | undefined, tag: string): boolean {
    return (def?.tags || []).includes(tag);
  }

  function resolveByTags(def: CatalogDef | null | undefined): string {
    if (!def) return "slash";
    const tags = def.tags || [];
  ...
  function resolveStyle(def: CatalogDef | null | undefined): ArenaAttackStyleDef {
  ...
  function resolveStyleId(def: CatalogDef | null | undefined): string {
    return resolveStyle(def).id;
  }

  function rollHits(style: ArenaAttackStyleDef, itemId: string): number {
  ...
  function createAttack(body: ArenaEquipBody, atkBase: Partial<ArenaAttackState> & { styleId?: string }): ArenaAttackState {
  ...
  function fireThoughtReaction(body: ArenaEquipBody, style: ArenaAttackStyleDef): void {
  ...
  function stepAttack(body: ArenaEquipBody, atk: ArenaAttackState, dt: number, vmin: number): boolean {
    const style = STYLES[atk.styleId] || STYLES.slash;
    const params = atk.styleParams || itemParams(body.itemId);
    const phases = style.phases;
    const dur = (phases[atk.phase as keyof typeof phases] || 0.2) * ATTACK_TIME_SCALE;
