 ...
 */
type TripleSupportRaw = {
  id: string;
  recommendedTriple: string;
  name?: string;
  icon?: string;
  tags?: string[];
  effects?: object[];
...
declare const BUILD_UNLOCK_CATALOG: Record<string, { mutation?: string; companion?: string }>;

const TRIPLE_SUPPORT_PATCHES: Record<string, { recommendedTriple: string; buildHints: string }> = {
  fire_staff: {
...
const TRIPLE_SUPPORT_CATALOG: Record<string, TripleSupportRaw> = {
  weapon_holy_mace: {
    id: "weapon_holy_mace",
    name: "Святой молот",
    icon: "🔨✨",
...
function normalizeTripleSupportItemDef(raw: TripleSupportRaw): TripleSupportRaw & { stats: { damage: number; defense: number; maxHp: number }; isContainer: boolean; craftOnly: boolean } {
  if (typeof defItem === "function") {
    ...
    }) as TripleSupportRaw & { stats: { damage: number; defense: number; maxHp: number }; isContainer: boolean; craftOnly: boolean };
  }
  return {
    ...raw,
...
function registerTripleSupportItems(): void {
...
function patchTripleSupportMetadata(): void {
  if (typeof ITEM_CATALOG === "undefined") return;
...
function getTripleBuildSpec(buildId: string) {
...
function scoreShopItemPickWeight(
  item: { recommendedTriple?: string; tags?: string[] },
  ctx: {
    unlockedBuilds?: Set<string>;
    loadoutItems?: object[];
...
function listTripleSupportItems(filters: { tripleId?: string } = {}): TripleSupportRaw[] {
...
function registerTripleSupportAssets(): void {
