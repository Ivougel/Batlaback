...
function getAvatarStackOrbitRing(team) {
  const mount = getStackOrbitMount(team);
  if (!mount) return null;
  const legacyStage = document.getElementById(team === "player" ? "player-avatar-slot" : "enemy-avatar-slot")?.querySelector(".avatar-hero-stage");
...
function syncStackOrbitFromBattle(battleState, opts = {}) {
  ...
  if (typeof BattleFxTier !== "undefined" && BattleFxTier.stackOrbitParticlesEnabled && !BattleFxTier.stackOrbitParticlesEnabled()) {
  ...
  stackOrbitLastSyncAt = now;
  const speedSig = typeof getEmojiOrbitDurationSec === "function" ? String(getEmojiOrbitDurationSec()) : "default";
...
function handleStackOrbitEvent(ev) {
  ...
  if (now - (stackFireThrottleAt[ev.side] || 0) < minGap) {
    const ring2 = getAvatarStackOrbitRing(ev.side);
    if (ring2) delete ring2.dataset.orbitSig;
    return;
  }
