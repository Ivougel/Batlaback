...
const MECHANIC_TAG_HINTS: Record<string, string> = {
...
const MECHANIC_TRIGGER_REPLACEMENTS: ReadonlyArray<readonly [string, string]> = [
...
function escapeMechanicHtml(text: unknown): string {
  return String(text ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
function escapeMechanicAttr(text: unknown): string {
  return escapeMechanicHtml(text).replace(/'/g, "&#39;");
}

/** «тег» → [тег], триггеры, «за каждый предмет [X]» → «… с [X]». */
function normalizeMechanicTags(text: unknown): string {
...
function formatMechanicTagHtml(label: string): string {
...
function formatMechanicTagsHtml(text: unknown, options: { normalize?: boolean } = {}): string {
...
function formatItemTagMechanic(tag: string): string {
