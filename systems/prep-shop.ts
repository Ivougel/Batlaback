 ...
 */
import type { PrepShopRuntime, PrepShopSideState } from "../types/game";

type ShopOpts = { isReroll?: boolean };
type ShopModifiers = { bonusUnique?: number } | null;
type CatalogDef = {
  id?: string;
  name?: string;
  cost?: number;
...
function getMaxShopSlots(): number {
  return typeof getPrepShopSlotCount === "function" ? getPrepShopSlotCount() : 5;
}

let rt!: PrepShopRuntime;

function registerPrepShopRuntime(deps: PrepShopRuntime): void {
  rt = deps;
}

function getShopContextForSide(side = rt.getPrepViewSide(), opts: ShopOpts = {}) {
...
function ensureSideShopArrays(st: PrepShopSideState): void {
...
function refreshShopSlotsForSide(side = rt.getPrepViewSide(), opts: ShopOpts = {}): number[] {
  const st = rt.getSideState(side);
  ...
  if (((ctx.shopModifiers as ShopModifiers)?.bonusUnique ?? 0) > 0 && ctx.bonusUniqueGranted) {
...
function toggleShopFreeze(index: number, side = rt.getPrepViewSide()): void {
  ...
  const name = typeof resolveShopEntryMeta === "function"
    ? (resolveShopEntryMeta(entryId ?? "")?.def as CatalogDef | undefined)?.name || entryId
    : (ITEM_CATALOG[entryId ?? ""] as CatalogDef | undefined)?.name || entryId;
  rt.log(st.shopFrozen[index] ? `📌 Закреплено: ${name}` : `📌 Снято закрепление: ${name}`);
  rt.playPrepSfx("prep_freeze");
  renderCommerceForMode(side);
}

function commitShopPurchase(index: number, side = rt.getPrepViewSide()): string | null {
  const st = rt.getSideState(side);
  const entryId = st.shop[index];
...
function buyFromShop(index: number, side = rt.getPrepViewSide()): void {
...
function buyFromShopForTdTower(
  index: number,
  side: string,
  applyEntry: (entry: { itemId: string; uid: string }) => boolean,
): boolean {
...
function getSellRefund(itemId: string, side = rt.getPrepViewSide()): number {
...
function creditItemSale(itemId: string, side = rt.getPrepViewSide()): void {
  if (!itemId) return;
...
function sellBenchEntry(index: number, side = rt.getPrepViewSide()): boolean {
  const st = rt.getSideState(side);
...
function renderShopPinButton(index: number, frozen: boolean, editable: boolean): string {
  if (!editable) return "";
  const label = frozen
...
function renderShopCostHTML(cost: number): string {
...
function renderShop(side = rt.getPrepViewSide(), containerEl: HTMLElement | null = null): void {
  ...
  let html = "";
  try {
    const entries = getShopDisplayEntries(side).filter((entry): entry is { index: number; entryId: string; itemId: string; cost: number } => Boolean(entry.itemId));
    if (entries.length) {
  ...
  el.querySelectorAll(".shop-card:not(.empty)").forEach((card: Element) => {
    bindItemTooltipEvents(card, (card as HTMLElement).dataset.itemId ?? "", null, "shop");
  });
  if (!editable) return;
  el.querySelectorAll(".shop-pin").forEach((btn: Element) => {
    btn.addEventListener("click", (e: Event) => {
      e.stopPropagation();
      toggleShopFreeze(+(btn as HTMLElement).dataset.pin!, side);
    });
  });
  ...
  el.querySelectorAll(".shop-card:not(.empty)").forEach((card: Element) => {
    ...
    if (!htmlCard.dataset.unaffordable) {
      ...
      htmlCard.addEventListener("mousedown", (e: Event) => {
        ...
        if (me.button !== 0 || (me.target as Element).closest(".shop-pin")) return;
        rt.beginPendingShopDrag(+htmlCard.dataset.index!, me, side);
      });
      htmlCard.addEventListener("click", (e: Event) => {
        const me = e as MouseEvent;
        ...
        if ((e.target as Element).closest(".shop-pin") || rt.getShopDidDrag()) {
...
function renderBench(side = rt.getPrepViewSide(), containerEl: HTMLElement | null = null): void {
  ...
  el.innerHTML = Array.from({ length: MAX_BENCH }, (_, i) => {
    ...
    const def = ITEM_CATALOG[b.itemId] as CatalogDef | undefined;
