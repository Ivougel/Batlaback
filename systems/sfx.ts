...
(function initGameSfx() {
  ...
  type ArpeggioOptions = ToneOptions & { duration?: number };
  type SfxFn = (options?: Record<string, unknown>) => void;
  type SfxThemeBuildApi = {
    tone: (freq: number, duration: number, opts?: ToneOptions) => void;
    noiseBurst: (duration: number, opts?: NoiseBurstOptions) => void;
    arpeggio: (notes: number[], gap?: number, opts?: ArpeggioOptions) => void;
  };
  ...
  function setSfxVolume(volume: number): number {
  ...
  function syncSfxVolumeUi(volume: number): void {
    const pct = Math.round(volume * 100);
    const slider = document.getElementById("settings-sfx-volume") as HTMLInputElement | null;
  ...
  function canPlay(id: string, minGapMs = 45): boolean {
  ...
  function noiseBurst(duration: number, { volume = 0.08, freq = 900, q = 0.7 }: NoiseBurstOptions = {}): void {
    const c = ensureCtx();
    if (!c || !master || getSfxVolume() <= 0) return;
    unlockAudio();
    const t0 = c.currentTime;
  ...
  function arpeggio(notes: number[], gap = 0.055, opts: ArpeggioOptions = {}): void {
  ...
  let SFX: Record<string, SfxFn> = {};

  function rebuildGameSfxTheme(themeId: string): void {
  ...
  function readInitialSoundThemeId(): string {
    try {
  ...
  function isInteractiveDisabled(el: Element | null): boolean {
    if (!el) return true;
  ...
  function resolveButtonSfx(el: Element | null): string | null {
    if (!el || isInteractiveDisabled(el)) return null;

    const sfxHost = el.closest("[data-sfx]") as HTMLElement | null;
  ...
  function findButtonSfxTarget(target: EventTarget | null): Element | null {
  ...
  function playButtonSfx(target: Element | string | null): boolean {
  ...
  function playUiHoverSfx(target: EventTarget | null): boolean {
  ...
  function playGameSfx(id: string, options: Record<string, unknown> = {}): boolean {
