...
const metrics = await page.evaluate(() => {
  const read = (side) => {
    ...
    const spec = document.getElementById(side === "player" ? "prep-character-spec-slot" : "prep-character-spec-slot-enemy");
    ...
    const r = (el) => {
      ...
      return { x: Math.round(b.x), y: Math.round(b.y), w: Math.round(b.width), h: Math.round(b.height), cx: Math.round(b.x + b.width / 2), cy: Math.round(b.y + b.height / 2) };
    };
    return { prep: r(prepImg), spec: r(spec), thought: r(thought), body: r(body), anchor, prepAnchor, glyph: body?.textContent?.trim() || null };
  };
  return {
